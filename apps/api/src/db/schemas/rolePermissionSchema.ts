...
export const rolePermissionSchema = pgTable('role_permission', {
  role_id: text('role_id').notNull().references(() => roleSchema.id),
  permission_id: text('permission_id').notNull().references(() => permissionSchema.id),
  created_at: timestamp('created_at').defaultNow().notNull(),
}, (table) => [ primaryKey({ columns: [table.role_id, table.permission_id] })]
);

export const rolePermissionRelations = relations(rolePermissionSchema, ({ one }) => ({
  role: one(roleSchema, {
